...
// Smart Resume File Handler — Redirects to Supabase Storage public URL
app.get('/uploads/resumes/:filename', (req, res) => {
  const rawParam = req.params.filename || '';
  let reqFile = rawParam;
  try { reqFile = decodeURIComponent(rawParam); } catch (_) {}

  if (reqFile.startsWith('http://') || reqFile.startsWith('https://')) {
    return res.redirect(reqFile);
  }

  const supaBaseUrl = process.env.SUPABASE_URL || 'https://gfyvfjgwnercvunvqzpk.supabase.co';
  const fileNameOnly = reqFile.split('/').pop();
  const publicUrl = `${supaBaseUrl}/storage/v1/object/public/resumes/${encodeURIComponent(fileNameOnly)}`;
